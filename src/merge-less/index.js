...
const { genHashCode } = require('../util');
...
const genProjectLess = (
  filePath,
  { isModule, publicLessPath, loadAny, cache, ignoreAntd, ignoreProLayout, ...rest },
) =>
  genModuleLess(filePath, { isModule, publicLessPath, ...rest }).then(async content => {
    if (cache === false) {
